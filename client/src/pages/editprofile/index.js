...
export default function EditProfile() {
  ...
  const handleProfile = async (e) => {
    ...
    const response = await axios
      // .put('https://investnow-backend.herokuapp.com/users/updateUserProfile/?email=' + localStorage.getItem
      .put(`${link}/users/updateUserProfile/?email=` + localStorage.getItem
      ("user"), userData)
      .catch((err) => {
        console.error(err);
      });
    ...
    try {
      if (response.status == 200) {
        // const userDetails = await axios.get("https://investnow-backend.herokuapp.com/users/fetchUserByEmail/?
         const userDetails = await axios.get(`${link}/users/fetchUserByEmail/?
